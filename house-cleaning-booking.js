...
(function(){
  ...
  const addonRates={
    refrigerator:35,oven:35,baseboards:40,
    movein:50,moveout:50,apartment:25,kitchen:50,bathroom:35,bedroom:25,
    livingroom:35,postparty:75,windows:50
  };
  const addonLabels={
    ...
    windows:'Interior Window Cleaning — +$50'
  };
  ...
  function buildFields(){const grid=document.querySelector('#booking .grid');if(!grid||$('hcHomeSize'))return;const date=$('date')?.parentElement;if(!date)return;
    ...
    const addons=field('hcAddons','Optional Cleaning Add-Ons','<div class="options" id="hcAddonOptions">'+Object.entries(addonLabels).map(([key,label])=>'<label><input type="checkbox" data-hc-addon="'+key+'"> '+label+'</label>').join('')+'</div><div class="note"><strong>Flat-rate add-ons:</strong> These options are added to the selected Standard Cleaning rate. Move-In, Move-Out, Apartment, Kitchen, Bathroom, Bedroom, Living Room, Post-Party, Refrigerator, Oven, Baseboards, and Interior Window Cleaning do not replace the home-size base rate.</div>');
  ...
  function selectedAddons(){let n=0;document.querySelectorAll('[data-hc-addon]:checked').forEach(e=>{const key=e.getAttribute('data-hc-addon');n+=Number(addonRates[key]||0);});const bedroom=$('hcSize')?.value;if(document.querySelector('[data-hc-addon="bedroom"]:checked')&&bedroom)n+=Math.max(0,Number(bedroom)-1)*25;return n;}
