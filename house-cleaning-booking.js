...
(function(){
  ...
  const addonRates={refrigerator:35,oven:35,baseboards:40,movein:50,moveout:50,apartment:25,kitchen:50,bathroom:35,bedroom:25,livingroom:35,postparty:75,windows:50};
  const addonLabels={refrigerator:'Inside Refrigerator — +$35',oven:'Inside Oven — +$35',baseboards:'Baseboards — +$40',movein:'Move-In Cleaning — +$50',moveout:'Move-Out Cleaning — +$50',apartment:'Apartment Cleaning Upgrade — +$25',kitchen:'Detailed Kitchen Cleaning — +$50',bathroom:'Detailed Bathroom Cleaning — +$35',bedroom:'Detailed Bedroom Cleaning — +$25 per bedroom',livingroom:'Detailed Living Room Cleaning — +$35',postparty:'Post-Party Cleanup — +$75',windows:'Interior Window Cleaning — +$50'};
  ...
  function addCleaningCategory(){
    if(document.getElementById('houseCleaningCategory'))return;
    const automotive=document.getElementById('automotive');
    if(!automotive)return;
    const section=document.createElement('section');
    section.className='category';
    section.id='houseCleaningCategory';
    section.innerHTML='<div class="category-head"><h2>House Cleaning</h2><p>Professional residential cleaning with simple flat-rate pricing. No hourly billing.</p></div><div class="services" style="display:block"><div class="service"><strong>House Cleaning</strong><span>Standard cleaning by home size, with optional Deep Clean and additional cleaning services.</span><button type="button" id="houseCleaningContinue">Book Online</button></div></div>';
    automotive.insertAdjacentElement('afterend',section);
    section.querySelector('#houseCleaningContinue').addEventListener('click',openCleaning);
  }
  ...
  function buildFields(){const grid=document.querySelector('#booking .grid');if(!grid||$('hcHomeSize'))return;const date=$('date')?.parentElement;if(!date)return;
    ...
    const addons=field('hcAddons','Optional Cleaning Add-Ons','<div class="options" id="hcAddonOptions">'+Object.entries(addonLabels).map(([key,label])=>'<label><input type="checkbox" data-hc-addon="'+key+'"> '+label+'</label>').join('')+'</div><div class="note"><strong>Flat-rate add-ons:</strong> Optional services are added to the selected home-size Standard Cleaning rate. They never replace the base rate.</div>');
    ...
    document.querySelectorAll('[data-hc-addon]').forEach(e=>e.addEventListener('change',()=>{if(isCleaning())renderEstimatePreview(false)}));
  }
  ...
  function selectedAddons(){let n=0;document.querySelectorAll('[data-hc-addon]:checked').forEach(e=>{const key=e.getAttribute('data-hc-addon');if(key==='bedroom')return;n+=Number(addonRates[key]||0);});const bedroom=$('hcSize')?.value;if(document.querySelector('[data-hc-addon="bedroom"]:checked')&&bedroom)n+=Number(bedroom)*25;return n;}
  function selectedAddonNames(){const names=[];document.querySelectorAll('[data-hc-addon]:checked').forEach(e=>{const key=e.getAttribute('data-hc-addon');names.push(addonLabels[key]||key);});return names;}
  function calculate(){const size=Number(val('hcSize'));if(!rates[size]){alert('Please select a home size.');return null;}const base=rates[size];const deep=$('hcDeepChoice')?.value==='yes'?deepRates[size]:0;const addons=selectedAddons();return{size,base,deep,addons,total:base+deep+addons,addonNames:selectedAddonNames()};}
  function renderEstimatePreview(open){const r=calculate();if(!r)return;const estimate=$('estimate');if(!estimate)return;let breakdown=$('cleaningEstimateBreakdown');if(!breakdown){breakdown=document.createElement('div');breakdown.id='cleaningEstimateBreakdown';estimate.querySelector('h2')?.after(breakdown);}breakdown.innerHTML='<div class="price-row"><span>Standard Cleaning — '+r.size+(r.size===5?'+':'')+' Bedroom'+(r.size===1?'':'s')+'</span><strong>'+money(r.base)+'</strong></div>'+(r.deep?'<div class="price-row"><span>Deep Clean</span><strong>+'+money(r.deep)+'</strong></div>':'')+(r.addons?'<div class="price-row"><span>Cleaning Add-Ons</span><strong>+'+money(r.addons)+'</strong></div>':'')+(r.addonNames.length?'<div class="note"><strong>Selected add-ons:</strong> '+r.addonNames.join(', ')+'</div>':'');$('laborPrice').textContent=money(r.base);$('partsPrice').textContent=money(r.deep+r.addons);$('totalPrice').textContent=money(r.total);const note=estimate.querySelector('.estimate-note');if(note)note.textContent='Flat-rate House Cleaning estimate. No hourly billing. Deep Clean and selected cleaning add-ons are optional extras. Your request is not a confirmed appointment and no payment is taken through this form.';if(open){estimate.classList.add('open');estimate.scrollIntoView({behavior:'smooth'});}}
  function interceptEstimate(e){if(!isCleaning())return;e.preventDefault();e.stopImmediatePropagation();renderEstimatePreview(true);}
  function init(){addServiceOption();addCleaningCategory();buildFields();$('service')?.addEventListener('change',()=>setCleaningVisibility(isCleaning()));const btn=document.querySelector('#booking button.continue');if(btn)btn.addEventListener('click',interceptEstimate,true);if(isCleaning())setCleaningVisibility(true);}
