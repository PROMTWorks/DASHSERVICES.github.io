/* DASH VEHICLE SELECTOR v13
   Existing DASH model/engine/trim entries are preserved exactly.
   New models get engine + trim data from the selected make's local
   versioned vehicle dataset. No NHTSA/vPIC/SafetyRatings dependency.
*/
(function(){
'use strict';
...
function extractVehicleData(brand,model,year,useYear){const engines=[],generations=[];for(const m of (((brand||{}).models||{}).model||[])){if(norm(m.name)!==norm(model))continue;for(const g of (((m.generations||{}).generation)||[])){let genUsed=false;for(const mod of (((g.modifications||{}).modification)||[])){if(useYear&&!inYear(mod,g,year))continue;if(mod.engine)engines.push(mod.engine);genUsed=true}if(genUsed&&g.name)generations.push(g.name)}}return {engines:unique(engines),generations:unique(generations)}}
...
async function realData(make,model,year){
 const local=localEntry(make,model);
 /* Never modify an existing DASH entry. Its engines/trims remain authoritative. */
 if(local)return {trims:unique(local.trims||[]),engines:unique(local.engines||[])};
 const brand=await loadBrand(make);
 let d=extractVehicleData(brand,model,year,true);
 /* New models are allowed to use their full versioned model record when the
    selected year has no matching modification, so they still get real data. */
 if(!d.engines.length&&!d.generations.length)d=extractVehicleData(brand,model,year,false);
 const styles=await loadStyles();
 const sm=slug(make),smo=slug(model),yr=Number(year);
 let trims=styles.filter(x=>x.make===sm&&x.model===smo&&styleActive(x.years,yr)).map(x=>x.name);
 if(!trims.length)trims=styles.filter(x=>x.make===sm&&x.model===smo).map(x=>x.name);
 return {trims:unique([...trims,...d.generations]),engines:unique(d.engines)};
}
async function init(){const year=get('year'),make=get('make'),model=get('model'),trim=get('trim'),engine=get('engine');if(!year||!make||!model||!trim||!engine||year.dataset.dashVehicleInit==='broad-v13')return;if(window.__DASH_BROAD_V13_INIT)return;window.__DASH_BROAD_V13_INIT=true;year.dataset.dashVehicleInit='broad-v13';
