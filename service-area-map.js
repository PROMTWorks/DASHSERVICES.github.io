(function(){
  ...
  var HOME_COPY='DASH Services is proud to serve Myrtle Beach, Conway, and communities throughout the surrounding Grand Strand area. We’re committed to providing dependable, convenient service wherever you need us.\n\nFrom everyday service needs to jobs that require a little more travel, we’re here to make the process simple from start to finish.\n\nExplore our services and find the right option for you on our Book a Service page.';

  function updateHomepageCopy(){
    var paragraphs=document.querySelectorAll('.visual-intro-copy p');
    paragraphs.forEach(function(p){
      if(p.textContent.indexOf('DASH Services is based around the Myrtle Beach and Conway area')!==-1 || p.textContent.indexOf('The homepage keeps this simple')!==-1){
        p.innerHTML='DASH Services is proud to serve Myrtle Beach, Conway, and communities throughout the surrounding Grand Strand area. We’re committed to providing dependable, convenient service wherever you need us.<br><br>From everyday service needs to jobs that require a little more travel, we’re here to make the process simple from start to finish.<br><br>Explore our services and find the right option for you on our <a href="book-service.html?v=20260816" style="color:#c62828;font-weight:900;text-decoration:underline">Book a Service</a> page.';
      }
    });
  }
  ...
  function loadScript(src,onload){
    ...
    s.src=src; s.async=true; s.defer=true; s.onload=onload;
  ...
  function start(){
    updateHomepageCopy();
    var config=document.createElement('script'); config.src='google-maps-config.js';
    config.onload=function(){var key=window.DASH_GOOGLE_MAPS_KEY;if(!key||key.indexOf('PASTE_')===0){console.warn('DASH map: add the new restricted Google Maps API key to google-maps-config.js.');return;}loadScript('https://maps.googleapis.com/maps/api/js?key='+encodeURIComponent(key)+'&v=weekly',initMap);};
    config.onerror=function(){console.warn('DASH map: google-maps-config.js could not load.');}; document.head.appendChild(config);
  }
